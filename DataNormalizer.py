...
class DataNormalizer:
    norm_config:dict
    stdparam:dict
    norm_class_size:dict
    def __init__(self,modelConfig,instanceConfig):
        self.stdparam = dict()
        self.norm_class_size = dict()
        self.dtype_sizes = dict()
        self.load_norm_config(modelConfig,instanceConfig)

    def load_norm_config(self,modelConfig,instanceConfig):
        with open(os.path.join(os.path.dirname(__file__),'norm_config.yml'), 'r') as yml:
            self.norm_config:dict = yaml.safe_load(yml)
            print("norm_config was loaded!")
        stdparam_dict:dict = self.norm_config['stdparam']
        stdparam_class_dict:dict = stdparam_dict['class']
        self.dtype_sizes:dict = self.norm_config['sizeparam']
        #classes = []
        ...
        for stdclass in stdparam_class_dict.keys():
            ...
            for stdclasskey in stdclasskeys:
                ...
                self.stdparam[stdclasskey] = mayvalue
        # print("Fields:",self.stdparam)
        self.class_norm:dict = self.norm_config['class']
        self.norm_class_size:dict = dict()
        for class_name,class_data in self.class_norm.items():
            # print(f"calcrate {class_name} size",class_data)
            self.norm_class_size[class_name] = self.searchClassNode(self.class_norm[class_name],class_name,[class_name])
            # print(f"{class_name} size =",self.norm_class_size[class_name])
        print(self.norm_class_size)
        
        

    def searchClassNode(self,top:dict,tab='',parents=[],ignores = []) -> int:
        total_size = 0
        if 'class' in top:
            if top['class'] in self.norm_class_size:
               node_size = self.norm_class_size[top['class']]
            else:
                node_size = self.searchClassNode(self.class_norm[top['class']],tab,parents,ignores.copy())
            # print(f"{tab} {top['class']} size = {node_size}")
                ...
                for igI in range(len(ignores)):
                    ...
                    if len(ignore) > 0:
                        if ignore[0] == parents[-1]:
                            if len(ignore) == 1:
                                # print(f"{tab} {ignore[0]} was ignored")
                                return total_size
                            else:
            ...
            if 'dtype' not in top:
                for node_key,node in top.items():
                    # print(f"{tab}\tsearch into",node_key,"/ignore:",new_ignores)
                    node_size = self.searchClassNode(node,tab+' > '+node_key,parents+[node_key],new_ignores)
                    # print(f"{tab}\t\t>> {node_key} size =",node_size)
                    total_size += node_size
            else:
                if top['norm'] != 'ignore':
                    datatype = top['dtype']
                    datanorm = top['norm']
                    if datatype in self.dtype_sizes:
                        if self.dtype_sizes[datatype] == 'relateChildren':
                                ...
                                if datanorm == 'objsnormalize':
                                    ...
                                    if isinstance(params,list):
                                        if all([param in self.norm_config for param in params]):
                                            for param in params:
                                                total_size += self.searchClassNode(self.class_norm[param],tab,parents,new_ignores) * paddings
                                    elif isinstance(params,dict):
                                        total_size = self.searchClassNode(params,tab,parents,new_ignores) * paddings
                        else:
                            total_size = int(self.dtype_sizes[datatype])
                    elif datanorm == 'onehot':
                        total_size = len(top['params'])
                    else:
                        total_size = int(self.dtype_sizes['default'])
        return total_size
...
class DataToPNG:
    ...
    def Convert(self,data:np.ndarray,h=None,w=None):
        b = int(math.sqrt(data.size/3))
        h = int(b if h == None else h)
        w = int((data.size/(3*h)) if w == None else w)
        d_cnv = (np.clip(data,self.vmin,self.vmax) - self.vmin)/(self.vmax-self.vmin) * 255
